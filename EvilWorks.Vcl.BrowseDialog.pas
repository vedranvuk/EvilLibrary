...
function TBrowseDialog.DoValidateFailedW(AEditText: PChar): integer;
begin
	{ Explicit conversion }
...
begin
	...
	if ShellVersion < $00040000 then
		raise Exception.Create('Shell not compatible');
...
function TBrowseDialog.GetEnumFlags(psf: IShellFolder; pidlFolder: PItemIDList; const phWnd: HWND; var pgrfFlags: DWORD): HResult;
var
	Flags: TBrowsableObjectClasses;
	Obj  : TBrowsableObjectClass;
begin
	{ (rb) Don't know for sure if pgrfFlags is initialized }
	Flags   := [];
	for Obj := low(TBrowsableObjectClass) to high(TBrowsableObjectClass) do
...
function TBrowseDialog.GetOwnerWindow: HWND;
var
	F: TCustomForm;
begin
...
function TBrowseDialog.GetRootDirectoryPath: string;
begin
	if FRootDirectory = fdNoSpecialFolder then
...
procedure TBrowseDialog.HookDialog;
begin
	if FDialogWindow <> 0 then
		FDefWndProc := Pointer(SetWindowLong(FDialogWindow, GWL_WNDPROC, longint(FObjectInstance)));
end;

function TBrowseDialog.IsRootDirectoryPathStored: boolean;
begin
	Result := (RootDirectory = fdNoSpecialFolder) and (FRootDirectoryPath > '');
end;

procedure TBrowseDialog.MainWndProc(var Msg: TMessage);
begin
	try
		Dispatch(Msg);
	except
...
procedure TBrowseDialog.SetExpanded(const APath: string);
begin
	if FDialogWindow <> 0 then
		{ Implicit conversion }
		SetExpandedW(APath);
...
procedure TBrowseDialog.SetExpanded(IDList: PItemIDList);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETEXPANDED, WParam(False), lParam(IDList));
end;

procedure TBrowseDialog.SetExpandedW(const APath: string);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETEXPANDED, WParam(True), lParam(PChar(APath)));
end;

procedure TBrowseDialog.SetOKEnabled(const Value: boolean);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_ENABLEOK, 0, lParam(Value));
end;

procedure TBrowseDialog.SetOKText(const AText: string);
begin
	if FDialogWindow <> 0 then
		{ Implicit conversion }
		SetOKTextW(AText);
...
procedure TBrowseDialog.SetOKTextW(const AText: string);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETOKTEXT, 0, lParam(PChar(AText)));
end;

procedure TBrowseDialog.SetOptions(const Value: TOptionsDir);
var
...
procedure TBrowseDialog.SetRootDirectory(const Value: TFromDirectory);
begin
...
procedure TBrowseDialog.SetRootDirectoryPath(const Value: string);
begin
	FRootDirectory     := fdNoSpecialFolder;
	FRootDirectoryPath := Value;
end;

procedure TBrowseDialog.SetSelection(const APath: string);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETSELECTION, WParam(True), lParam(Pointer(APath)));
end;

procedure TBrowseDialog.SetSelection(IDList: PItemIDList);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETSELECTION, WParam(False), lParam(IDList));
end;

procedure TBrowseDialog.SetStatusText(const AText: string);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETSTATUSTEXT, 0, lParam(Pointer(AText)));
end;

procedure TBrowseDialog.SetStatusTextW(const AText: string);
begin
	if FDialogWindow <> 0 then
		SendMessage(FDialogWindow, BFFM_SETSTATUSTEXTW, 0, lParam(PChar(AText)));
end;

function TBrowseDialog.ShouldShow(psf: IShellFolder; pidlFolder, pidlItem: PItemIDList): HResult;
var
	StrRet: TStrRet;
begin
...
procedure TBrowseDialog.UpdateStatusText(AText: string);
const
	cStatusLabel = $3743;
var
...
begin
	...
	begin
		...
		begin
			...
			AText := MinimizeFileName(AText, LCanvas, (WindowRect.Right - WindowRect.Left) - (ItemRect.Left - WindowRect.Left) * 2 - 8);
			if Application.MainForm = nil then
				LCanvas.Free;
		end;
	end;
...
procedure TBrowseDialog.WMShowWindow(var Msg: TMessage);
begin
...
procedure TBrowseDialog.WMSize(var Msg: TWMSize);
var
	BtnSize   : TRect;
	WindowSize: TRect;
begin
